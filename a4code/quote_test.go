...
func TestQuoteDepthOptions(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			restrictedQuoteDepth: func() *int { i := 0; return &i }(),
			want:                 "",
		},
		{
		...
		},
		{
			...
			input:                "[quoteof \"A\" [quoteof \"B\" content]]",
			restrictedQuoteDepth: func() *int { i := 1; return &i }(),
		...
		},
		{
			...
			input:               "[quoteof \"A\" content]",
			truncatedQuoteDepth: func() *int { i := 0; return &i }(),
			want:                "[quoteof \"user\" [quoteof \"A\"]]\n",
		},
		{
			name:                 "TruncatedDepth1_TruncateDepth2",
			input:                "[quoteof \"A\" [quoteof \"B\" content]]",
			truncatedQuoteDepth:  func() *int { i := 1; return &i }(),
			restrictedQuoteDepth: func() *int { i := 2; return &i }(),
