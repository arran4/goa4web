...
func (cd *CoreData) BlogCommentThread(ops ...lazy.Option[int32, *db.GetThreadLastPosterAndPermsForUserRow]) (*db.GetThreadLastPosterAndPermsForUserRow, error) {
...
func (cd *CoreData) UpdateBlogReply(commentID, commenterID, languageID int32, text string) error {
	...
	rows, err := cd.queries.UpdateCommentForEditor(cd.ctx, db.UpdateCommentForEditorParams{
		...
		CommentID:    commentID,
		EditorID:     commenterID,
		EditorUserID: sql.NullInt32{Int32: commenterID, Valid: commenterID != 0},
	})
	if err != nil {
		return err
	}
