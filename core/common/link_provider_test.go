...
func (m *MockQuerier) GetExternalLink(ctx context.Context, urlHash string) (*db.ExternalLink, error) {
	for _, link := range m.Links {
		if GenerateURLHash(CanonicalizeExternalURL(link.Url)) == urlHash {
			return link, nil
		}
	}
