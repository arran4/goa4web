		...
		if cd.cache.testGrants != nil {
			for _, g := range cd.cache.testGrants {
				if g.Section == section && g.Action == action &&
					...
					// Mirror SQL logic: if it's a specific private forum check, don't allow generic matches
					if (section == "privateforum" || section == "privateforum_thread") && itemID != 0 {
						if !g.ItemID.Valid || !g.UserID.Valid {
							continue
						}
					}

					if !g.ItemID.Valid || g.ItemID.Int32 == itemID || g.ItemID.Int32 == 0 {
						return true
					}
				}
			}
