...
INSERT INTO roles (name, can_login, is_admin) VALUES
  ...
  ('rejected', 0, 0),
  ('administrator', 1, 1)
ON CONFLICT(name) DO UPDATE SET can_login = excluded.can_login, is_admin = excluded.is_admin;
...
SELECT CURRENT_TIMESTAMP, g.role_id, g.section, NULL, 'allow', 'label', 1
FROM grants g
...
INSERT INTO grants (created_at, role_id, section, item, rule_type, action, active)
SELECT CURRENT_TIMESTAMP, r_admin.id, 'role', NULL, 'allow', 'moderator', 1
FROM roles r_admin
