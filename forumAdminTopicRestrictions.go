...
func forumAdminTopicRestrictionLevelPage(w http.ResponseWriter, r *http.Request) {
	...
	type Data struct {
		*CoreData
		Restrictions []*getTopicRestrictionsRow
	}
	...
	if err := getCompiledTemplates().ExecuteTemplate(w, "forumAdminTopicRestrictionLevelPage.gohtml", data); err != nil {
...
func forumAdminTopicRestrictionLevelDeletePage(w http.ResponseWriter, r *http.Request) {
	...
	if err := queries.deleteTopicRestrictions(r.Context(), int32(topicId)); err != nil {
		http.Redirect(w, r, "?error="+err.Error(), http.StatusTemporaryRedirect)
		return
	}
