...
func TestDynamicBreadcrumb(t *testing.T) {
    ...
    if crumbs[0].Title != "Admin" { t.Errorf("1st crumb wrong") }
    if crumbs[1].Title != "Roles" { t.Errorf("2nd crumb wrong") }
    if crumbs[2].Title != "Role Moderator" { t.Errorf("3rd crumb wrong: %s", crumbs[2].Title) }
}
