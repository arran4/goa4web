...
func TestHappyPathAdminMaintenancePageListsTopics(t *testing.T) {
	qs := testhelpers.NewQuerierStub()
	qs.AdminListTopicsWithUserGrantsNoRolesReturns = []*db.AdminListTopicsWithUserGrantsNoRolesRow{
	...
	if len(qs.AdminListTopicsWithUserGrantsNoRolesCalls) != 1 {
		t.Fatalf("expected 1 topic listing call, got %d", len(qs.AdminListTopicsWithUserGrantsNoRolesCalls))
	}
	if include, ok := qs.AdminListTopicsWithUserGrantsNoRolesCalls[0].(bool); !ok || !include {
		t.Fatalf("expected includeAdmin=true, got %#v", qs.AdminListTopicsWithUserGrantsNoRolesCalls[0])
	}
}
