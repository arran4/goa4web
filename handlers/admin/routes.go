...
func RegisterRoutes(ar *mux.Router) {
	...
	nar.HandleFunc("/users/permissions", news.NewsAdminUserLevelsPage).Methods("GET")
	nar.HandleFunc("/users/permissions", tasks.Action(newsUserAllow)).Methods("POST").MatcherFunc(newsUserAllow.Matcher())
	nar.HandleFunc("/users/permissions", tasks.Action(newsUserRemove)).Methods("POST").MatcherFunc(newsUserRemove.Matcher())
