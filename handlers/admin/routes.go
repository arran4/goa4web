...
func RegisterRoutes(ar *mux.Router) {
	...
	ar.HandleFunc("/email/queue", AdminEmailQueueResendActionPage).Methods("POST").MatcherFunc(ResendQueueTask.Match)
	ar.HandleFunc("/email/queue", AdminEmailQueueDeleteActionPage).Methods("POST").MatcherFunc(DeleteQueueTask.Match)
	ar.HandleFunc("/email/template", AdminEmailTemplatePage).Methods("GET")
	ar.HandleFunc("/email/template", AdminEmailTemplateSaveActionPage).Methods("POST").MatcherFunc(SaveTemplateTask.Match)
	ar.HandleFunc("/email/template", AdminEmailTemplateTestActionPage).Methods("POST").MatcherFunc(TestTemplateTask.Match)
	ar.HandleFunc("/dlq", AdminDLQPage).Methods("GET")
	ar.HandleFunc("/dlq", AdminDLQAction).Methods("POST").MatcherFunc(DeleteDLQTask.Match)
	ar.HandleFunc("/notifications", AdminNotificationsPage).Methods("GET")
	ar.HandleFunc("/notifications", AdminNotificationsMarkReadActionPage).Methods("POST").MatcherFunc(MarkReadTask.Match)
	ar.HandleFunc("/notifications", AdminNotificationsPurgeActionPage).Methods("POST").MatcherFunc(PurgeNotificationsTask.Match)
	ar.HandleFunc("/notifications", AdminNotificationsSendActionPage).Methods("POST").MatcherFunc(SendNotificationTask.Match)
	...
	ar.HandleFunc("/announcements", AdminAnnouncementsAddActionPage).Methods("POST").MatcherFunc(AddAnnouncementTask.Match)
	ar.HandleFunc("/announcements", AdminAnnouncementsDeleteActionPage).Methods("POST").MatcherFunc(DeleteAnnouncementTask.Match)
	ar.HandleFunc("/ipbans", AdminIPBanPage).Methods("GET")
	ar.HandleFunc("/ipbans", AdminIPBanAddActionPage).Methods("POST").MatcherFunc(AddIPBanTask.Match)
	ar.HandleFunc("/ipbans", AdminIPBanDeleteActionPage).Methods("POST").MatcherFunc(DeleteIPBanTask.Match)
	...
	nar.HandleFunc("/users/levels", news.NewsAdminUserLevelsAllowActionPage).Methods("POST").MatcherFunc(NewsUserAllowTask.Match)
	nar.HandleFunc("/users/levels", news.NewsAdminUserLevelsRemoveActionPage).Methods("POST").MatcherFunc(NewsUserRemoveTask.Match)
