...
func RegisterRoutes(ar *mux.Router) {
	...
	ar.HandleFunc("/email/queue", ResendQueueTask.Action).Methods("POST").MatcherFunc(ResendQueueTask.Matcher())
	ar.HandleFunc("/email/queue", DeleteQueueTask.Action).Methods("POST").MatcherFunc(DeleteQueueTask.Matcher())
	ar.HandleFunc("/email/template", AdminEmailTemplatePage).Methods("GET")
	ar.HandleFunc("/email/template", SaveTemplateTask.Action).Methods("POST").MatcherFunc(SaveTemplateTask.Matcher())
	ar.HandleFunc("/email/template", TestTemplateTask.Action).Methods("POST").MatcherFunc(TestTemplateTask.Matcher())
	ar.HandleFunc("/dlq", AdminDLQPage).Methods("GET")
	ar.HandleFunc("/dlq", DeleteDLQTask.Action).Methods("POST").MatcherFunc(DeleteDLQTask.Matcher())
	ar.HandleFunc("/notifications", AdminNotificationsPage).Methods("GET")
	ar.HandleFunc("/notifications", MarkReadTask.Action).Methods("POST").MatcherFunc(MarkReadTask.Matcher())
	ar.HandleFunc("/notifications", PurgeNotificationsTask.Action).Methods("POST").MatcherFunc(PurgeNotificationsTask.Matcher())
	ar.HandleFunc("/notifications", SendNotificationTask.Action).Methods("POST").MatcherFunc(SendNotificationTask.Matcher())
