...
func (h *Handlers) RegisterRoutes(ar *mux.Router, cfg *config.RuntimeConfig, navReg *navpkg.Registry) {
	...
	navReg.RegisterAdminControlCenter("Core", "Role SQL Loader", "/admin/roles/load", 26)
	...
	navReg.RegisterAdminControlCenter("Core", "DB Status", "/admin/db/status", 31)
	navReg.RegisterAdminControlCenter("Core", "DB Schema", "/admin/db/schema", 156)
	navReg.RegisterAdminControlCenter("Core", "DB Migrations", "/admin/db/migrations", 157)
	navReg.RegisterAdminControlCenter("Core", "Link Tools", "/admin/links/tools", 31)
	...
	navReg.RegisterAdminControlCenter("Core", "Subscription Templates", "/admin/subscriptions/templates", 95)
	...
	navReg.RegisterAdminControlCenter("Core", "Template Export", "/admin/templates/export", 121)
	...
	navReg.RegisterAdminControlCenter("Core", "Config Export", "/admin/config/as-cli", 151)
	navReg.RegisterAdminControlCenter("Core", "Config Explain", "/admin/config/explain", 151)
	...
	navReg.RegisterAdminControlCenter("Core", "DB Backup", "/admin/db/backup", 154)
	navReg.RegisterAdminControlCenter("Core", "DB Restore", "/admin/db/restore", 155)
	...
	ar.HandleFunc("", handlers.TaskHandler(&AdminPageTask{})).Methods("GET")
	ar.HandleFunc("/", handlers.TaskHandler(&AdminPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/roles", handlers.TaskHandler(&AdminRolesPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/external-links", handlers.TaskHandler(&AdminExternalLinksPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/email/queue", handlers.TaskHandler(&AdminEmailPageTask{})).Methods("GET")
	ar.HandleFunc("/email/failed", handlers.TaskHandler(&AdminEmailPageTask{})).Methods("GET")
	ar.HandleFunc("/email/sent", handlers.TaskHandler(&AdminEmailPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/user", handlers.TaskHandler(&AdminUserListPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/announcements", handlers.TaskHandler(&AdminAnnouncementsPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/comments", handlers.TaskHandler(&AdminCommentsPageTask{})).Methods("GET")
	...
	ar.HandleFunc("/settings", handlers.TaskHandler(&AdminSiteSettingsPageTask{ConfigFile: h.ConfigFile})).Methods("GET", "POST")
