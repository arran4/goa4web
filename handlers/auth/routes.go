...
func RegisterRoutes(r *mux.Router, _ *config.RuntimeConfig) []nav.RouterOptions {
	...
	rr.HandleFunc("", handlers.WithNoCache(handlers.TaskHandler(registerTask))).Methods("POST").MatcherFunc(gml.Not(handlers.RequiresAnAccount())).MatcherFunc(registerTask.Matcher())

	lr := r.PathPrefix("/login").Subrouter()
	lr.HandleFunc("", handlers.WithNoCache(loginTask.Page)).Methods("GET")
	lr.HandleFunc("", handlers.WithNoCache(handlers.TaskHandler(loginTask))).Methods("POST").MatcherFunc(loginTask.Matcher())
	lr.HandleFunc("/verify", handlers.WithNoCache(handlers.TaskHandler(verifyPasswordTask))).Methods("POST").MatcherFunc(verifyPasswordTask.Matcher())

	lr.HandleFunc("/passkey/begin", HasWebAuthn(handlers.WithNoCache(loginPasskeyBegin))).Methods("GET")
	lr.HandleFunc("/passkey/finish", handlers.WithNoCache(HasWebAuthn(loginPasskeyFinish))).Methods("POST")
