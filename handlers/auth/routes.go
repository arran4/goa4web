...
func RegisterRoutes(r *mux.Router, _ *config.RuntimeConfig, _ *nav.Registry) {
	...
	rr.HandleFunc("", handlers.NoCache(registerTask.Page)).Methods("GET").MatcherFunc(Not(handlers.RequiresAnAccount()))
	...
	lr.HandleFunc("", handlers.NoCache(loginTask.Page)).Methods("GET").MatcherFunc(Not(handlers.RequiresAnAccount()))
	...
	fr.HandleFunc("", handlers.NoCache(forgotPasswordTask.Page)).Methods("GET").MatcherFunc(Not(handlers.RequiresAnAccount()))
