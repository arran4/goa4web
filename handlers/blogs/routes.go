...
func RegisterRoutes(r *mux.Router, _ *config.RuntimeConfig, navReg *navpkg.Registry) {
	...
	br.HandleFunc("/add", handlers.EnforceGrant(addBlogTask.Page, nil, "blogs", "entry", "post", 0)).Methods("GET")
	br.HandleFunc("/add", handlers.EnforceGrant(handlers.TaskHandler(addBlogTask), nil, "blogs", "entry", "post", 0)).Methods("POST").MatcherFunc(addBlogTask.Matcher())
