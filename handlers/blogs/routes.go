...
func RegisterRoutes(r *mux.Router) {
	...
	br.HandleFunc("/", Page).Methods("GET")
	br.HandleFunc("/add", AddBlogTask.Page).Methods("GET").MatcherFunc(auth.RequiredAccess("writer", "administrator"))
	br.HandleFunc("/add", AddBlogTask.Action).Methods("POST").MatcherFunc(auth.RequiredAccess("writer", "administrator")).MatcherFunc(AddBlogTask.Match)
	...
	br.Handle("/blog/{blog}/edit", RequireBlogAuthor(http.HandlerFunc(EditBlogTask.Page))).Methods("GET").MatcherFunc(auth.RequiredAccess("writer", "administrator"))
	br.Handle("/blog/{blog}/edit", RequireBlogAuthor(http.HandlerFunc(EditBlogTask.Action))).Methods("POST").MatcherFunc(auth.RequiredAccess("writer", "administrator")).MatcherFunc(EditBlogTask.Match)
