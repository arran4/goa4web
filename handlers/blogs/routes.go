...
func RegisterRoutes(r *mux.Router, _ *config.RuntimeConfig) []navpkg.RouterOptions {
	...
	br.Handle("/add", RequireBlogAddGrant(http.HandlerFunc(addBlogTask.Page))).Methods("GET")
	br.Handle("/add", RequireBlogAddGrant(http.HandlerFunc(handlers.TaskHandler(addBlogTask)))).Methods("POST").MatcherFunc(addBlogTask.Matcher())
