...
func TestAdminTopicDeleteConfirmPage(t *testing.T) {
	...
	// Set the templates directory to the relative path from handlers/forum/ to core/templates
	cfg.TemplatesDir = "../../core/templates"

	cd := common.NewCoreData(context.Background(), db.New(conn), cfg)
	...
	// Verify cascade checkbox is present
	if !strings.Contains(body, `name="cascade"`) {
		t.Errorf("expected cascade checkbox in body")
	}
}
...
func TestAdminTopicDeletePage_WithCascade(t *testing.T) {
	...
	// Use regex for the complex multi-table delete
	// The query is: DELETE forumthread, comments, comments_search FROM forumthread LEFT JOIN ... WHERE ...
	mock.ExpectExec(regexp.QuoteMeta("DELETE forumthread, comments, comments_search FROM forumthread")).
		WithArgs(123).
