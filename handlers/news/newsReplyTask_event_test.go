...
func TestNewsReplyTaskEventData(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO comments").
		WithArgs(int32(1), uid, pthid, sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), "news", sqlmock.AnyArg(), int32(pid), sqlmock.AnyArg(), uid).
		WillReturnResult(sqlmock.NewResult(5, 1))

	mock.ExpectExec(regexp.QuoteMeta("INSERT IGNORE INTO content_private_labels")).
		WithArgs("news", int32(pid), uid, "new", true).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(regexp.QuoteMeta("INSERT IGNORE INTO content_private_labels")).
		WithArgs("news", int32(pid), uid, "unread", true).
		WillReturnResult(sqlmock.NewResult(0, 1))
