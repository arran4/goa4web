...
func RegisterRoutes(r *mux.Router) {
	...
	nr.HandleFunc("/news/{post}", NewsPostReplyActionPage).Methods("POST").MatcherFunc(auth.RequiresAnAccount()).MatcherFunc(ReplyTask.Match)
	...
	nr.HandleFunc("/news/{post}", NewsPostEditActionPage).Methods("POST").MatcherFunc(auth.RequiredAccess("writer", "administrator")).MatcherFunc(EditTask.Match)
	nr.HandleFunc("/news/{post}", NewsPostNewActionPage).Methods("POST").MatcherFunc(auth.RequiredAccess("writer", "administrator")).MatcherFunc(NewPostTask.Match)
	nr.HandleFunc("/news/{post}/announcement", NewsAnnouncementActivateActionPage).Methods("POST").MatcherFunc(auth.RequiredAccess("administrator")).MatcherFunc(AnnouncementAddTask.Match)
	nr.HandleFunc("/news/{post}/announcement", NewsAnnouncementDeactivateActionPage).Methods("POST").MatcherFunc(auth.RequiredAccess("administrator")).MatcherFunc(AnnouncementDeleteTask.Match)
	...
	nr.HandleFunc("/users/permissions", NewsUsersPermissionsPermissionUserAllowPage).Methods("POST").MatcherFunc(auth.RequiredAccess("administrator")).MatcherFunc(UserAllowTask.Match)
	nr.HandleFunc("/users/permissions", NewsUsersPermissionsDisallowPage).Methods("POST").MatcherFunc(auth.RequiredAccess("administrator")).MatcherFunc(UserDisallowTask.Match)
}
