...
func RegisterRoutes(r *mux.Router, cfg *config.RuntimeConfig) []navpkg.RouterOptions {
	...
	pr.Handle("/topic/{topic}", RequirePrivateTopicAccess(http.HandlerFunc(TopicPage))).Methods(http.MethodGet).MatcherFunc(handlers.RequiresAnAccount())
	...
	pr.Handle("/topic/{topic}/thread/{thread}/replies", forumhandlers.RequireThreadAndTopic(http.HandlerFunc(forumhandlers.ReplyThreadsPage))).Methods(http.MethodGet).MatcherFunc(handlers.RequiresAnAccount())
