...
const getAllBlogEntriesByUserAdmin = `-- name: GetAllBlogEntriesByUserAdmin :many
...
type GetAllBlogEntriesByUserAdminRow struct {
	Idblogs            int32
	ForumthreadID      sql.NullInt32
	UsersIdusers       int32
	LanguageIdlanguage int32
...
func (q *Queries) GetAllBlogEntriesByUserAdmin(ctx context.Context, usersIdusers int32) ([]*GetAllBlogEntriesByUserAdminRow, error) {
	rows, err := q.db.QueryContext(ctx, getAllBlogEntriesByUserAdmin, usersIdusers)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []*GetAllBlogEntriesByUserAdminRow
	for rows.Next() {
		var i GetAllBlogEntriesByUserAdminRow
		if err := rows.Scan(
			&i.Idblogs,
			&i.ForumthreadID,
			&i.UsersIdusers,
