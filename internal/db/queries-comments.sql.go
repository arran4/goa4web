...
const appendToCommentForPoster = `-- name: AppendToCommentForPoster :exec
UPDATE comments
SET text = ?, written = ?
WHERE idcomments = ?
`

type AppendToCommentForPosterParams struct {
	Text       sql.NullString
	Written    sql.NullTime
	Idcomments int32
}

func (q *Queries) AppendToCommentForPoster(ctx context.Context, arg AppendToCommentForPosterParams) error {
	_, err := q.db.ExecContext(ctx, appendToCommentForPoster, arg.Text, arg.Written, arg.Idcomments)
	return err
}
...
const getLastCommentByThreadIdForPoster = `-- name: GetLastCommentByThreadIdForPoster :one
SELECT idcomments, forumthread_id, users_idusers, language_id, written, text, timezone, deleted_at, last_index FROM comments
WHERE forumthread_id = ?
ORDER BY Idcomments DESC
LIMIT 1
`

func (q *Queries) GetLastCommentByThreadIdForPoster(ctx context.Context, forumthreadID int32) (*Comment, error) {
	row := q.db.QueryRowContext(ctx, getLastCommentByThreadIdForPoster, forumthreadID)
	var i Comment
	err := row.Scan(
		&i.Idcomments,
		&i.ForumthreadID,
