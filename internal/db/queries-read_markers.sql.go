...
const checkIfOthersReadCommentForPoster = `-- name: CheckIfOthersReadCommentForPoster :one
SELECT 1 FROM content_read_markers
WHERE item = ? AND item_id = ? AND user_id != ? AND last_comment_id >= ?
LIMIT 1
`

type CheckIfOthersReadCommentForPosterParams struct {
	Item          string
	ItemID        int32
	UserID        int32
	LastCommentID int32
...
func (q *Queries) CheckIfOthersReadCommentForPoster(ctx context.Context, arg CheckIfOthersReadCommentForPosterParams) (int32, error) {
	row := q.db.QueryRowContext(ctx, checkIfOthersReadCommentForPoster,
		arg.Item,
		arg.ItemID,
		arg.UserID,
		arg.LastCommentID,
