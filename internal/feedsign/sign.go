...
// The resulting URL will be "/blogs/rss/u/{username}?ts={ts}&sig={sig}&{query}"
func (s *Signer) SignedURL(path, query, username string, exp ...time.Time) string {
	...
	// We append /u/{username} to the path.
	res := fmt.Sprintf("%s/u/%s?ts=%d&sig=%s", path, url.QueryEscape(username), ts, sig)
	if query != "" {
		res += "&" + query
	}
	return res
