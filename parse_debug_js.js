...
function calculateSourceOffset(node, offset) {
    if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (parent && (parent.hasAttribute('data-comment-offset') || parent.hasAttribute('data-start-pos'))) {
        ...
        if (offset < node.childNodes.length) {
            ...
            if (child.nodeType === Node.ELEMENT_NODE && (child.hasAttribute('data-comment-offset') || child.hasAttribute('data-start-pos'))) {
    ...
    let current = node;
    while (current) {
        if (current.nodeType === Node.ELEMENT_NODE && (current.hasAttribute('data-comment-offset') || current.hasAttribute('data-start-pos'))) {
