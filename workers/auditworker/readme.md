...
Package `auditworker` implements a specific background worker (`auditworker`). Workers are detached, asynchronous processors that respond to eventbus notifications, manage scheduled tasks, or process queues (like email or external link scanning). They handle heavy, long-running, or non-blocking tasks that should not delay the HTTP request-response cycle.
...
```go
import "goa4web/workers/auditworker"
```
